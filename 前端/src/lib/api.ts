...
export const adminApi = {
  ...
  getWordStats: async (word: string, days: number = 7): Promise<{
    word: string;
    total_users: number;
    ...
    const response = await fetch(`/api/admin/analytics/word-stats?word=${encodeURIComponent(word)}&days=${days}`, {
      headers: adminHeaders(),
    });
    return response.json();
  },
  ...
  getNewsCategory: async (): Promise<NewsCategoryData> => {
    const response = await fetch('/api/admin/analytics/news-category', {
      headers: adminHeaders(),
    });
    return response.json();
  },
