...
def calculate_coverage(content: str, summary: str) -> int:
    """计算摘要覆盖率 - 摘要中包含的关键词在原文中的覆盖比例"""
    if not content or not summary:
        return 0
    
    content_keywords = set(_get_keywords(content, max_keywords=30))
    summary_keywords = set(_get_keywords(summary, max_keywords=20))
    
    if not content_keywords:
        return 0
    
    matched_keywords = content_keywords.intersection(summary_keywords)
    coverage = round((len(matched_keywords) / len(content_keywords)) * 100)
