...
def list_news(
    ...
    with Session(engine) as db:
        stmt = select(News).where(News.review_status == "published")
        if ids:
        ...
        items = db.exec(
            stmt.order_by(col(News.published_at).desc().nulls_last(), News.id.desc()).offset(offset).limit(page_size)  # type: ignore[attr-defined]
        ).all()
...
def get_news(news_id: int) -> NewsItemOut:
    with Session(engine) as db:
        ...
        if not news or news.review_status != "published":
...
def list_categories() -> dict[str, list[str]]:
    with Session(engine) as db:
        rows = db.exec(select(News.category).where(News.review_status == "published").distinct()).all()
...
def stats() -> dict[str, Any]:
    with Session(engine) as db:
        total = len(db.exec(select(News).where(News.review_status == "published")).all())
        trending = len(db.exec(select(News).where(News.review_status == "published").where(News.is_trending == 1)).all())
        source_count = len(db.exec(select(CrawlSource)).all())
        log_count = len(db.exec(select(CrawlLog)).all())
        latest = db.exec(
            select(News).where(News.review_status == "published").order_by(col(News.published_at).desc().nulls_last(), News.id.desc()).limit(1)  # type: ignore[attr-defined]
        ).first()
        return {
            "total_news": total,
