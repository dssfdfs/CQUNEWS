const API_KEY = 'sk-cf5b7fb15a554d84b5610e406b7bb51c';
...
export async function callDeepSeek(model: string, messages: Message[], temperature = 0.7, max_tokens = 2000): Promise<string> {
...
4. ${language === '中文' ? '使用中文' : 'Use English'}`;
  ...
  return callDeepSeek('deepseek-v4-flash', [
...
${language === '中文' ? '请使用中文' : 'Please use English'}，每个标题一行，按顺序输出。`;
  ...
  const result = await callDeepSeek('deepseek-v4-flash', [
    { role: 'system', content: systemPrompt },
    { role: 'user', content: userPrompt },
  ]);
