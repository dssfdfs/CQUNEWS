...
from .models import AdminUser, AuditLog, EmailVerificationCode, Feedback, LoginHistory, News, Notification, PasswordResetToken, SystemConfig, User, UserBehavior, UserFavoriteNews, UserHistory, UserMaterial, UserProfile, UserProfileHistory, UserSettings
...
def get_news_category_distribution(
    ...
    category_labels: Record[str, str] = {
        "tech": "科技",
        "finance": "财经",
        "sports": "体育",
        "entertainment": "娱乐",
...
def get_users(
    ...
    for user in users:
        ...
        result.append({
            ...
            "last_active": to_beijing_time(last_active),
        })
...
@router.get("/config/api-configs")
def get_api_configs(db: Session = Depends(get_session), admin: AdminUser = Depends(get_admin_user)):
...
@router.put("/config/api-config")
def update_api_config(
    request: ApiConfigUpdateRequest,
    db: Session = Depends(get_session),
    admin: AdminUser = Depends(get_admin_user),
):
    config = db.exec(select(SystemConfig).where(SystemConfig.key == request.key)).first()
    if config:
...
def get_user_profile_word_cloud(
    ...
    stopwords = {
        ...
        "澎湃新闻", "新华网", "新华社", "中国新闻网", "人民网", "央视新闻", "光明网", "中国日报", "环球网",
        "界面新闻", "财经网", "科技日报", "经济日报", "法制日报", "新京报", "南方都市报", "北京日报",
        "分享到", "字号", "大字体", "小字体", "组图", "听全文", "责任编辑", "作者", "发布时间",
        "首页", "国内新闻", "国际新闻", "专题报道", "来源", "字体", "全屏", "打印", "收藏",
        "专题首页", "最新播报", "评论解读", "国新办举行", "系列主题新闻发布会", "新华网首页",
        "刘阳禾", "张令旗", "陈立泉", "曲靖通", "陈斌华", "平凡之光", "大有力量",
        "深入学习贯彻", "习近平党建思想", "中国绿色转型", "可持续发展", "世界文化遗产",
        "澎湃新闻记者", "澎湃新闻特约撰稿", "澎湃防务", "澎湃新闻网",
        "新华网专题报道", "新华网时政", "新华网科技", "新华网国际",
        "凤凰网", "网易新闻", "腾讯新闻", "新浪新闻", "搜狐新闻", "今日头条",
        "浏览", "阅读", "点击", "访问", "查看", "页面", "网站", "链接", "网页",
        "正文", "全文", "字号设置", "大中小", "返回首页", "返回顶部", "上一篇", "下一篇",
        "相关新闻", "推荐阅读", "延伸阅读", "点击排行", "热门推荐", "分享", "收藏", "评论",
    }
    ...
    def extract_keywords_from_text(text):
        ...
        for length in range(6, 1, -1):
            for i in range(len(text_clean) - length + 1):
                substring = text_clean[i:i+length]
                if substring in all_keywords:
    ...
    for behavior in behaviors:
        weight = 5 if behavior.action_type == "generate" else 3
        if behavior.action_type == "redirect":
            weight = 4
        ...
        if behavior.extra_data:
            try:
                ...
                if 'category' in extra and extra['category']:
                    word_counts[extra['category']] = word_counts.get(extra['category'], 0) + weight
                if 'title' in extra and extra['title']:
                    keywords = extract_keywords_from_text(extra['title'])
                    for kw in keywords:
                        word_counts[kw] = word_counts.get(kw, 0) + weight // 2
            except:
                pass
        ...
        if behavior.target_id:
            ...
            if news:
                if news.category:
                    word_counts[news.category] = word_counts.get(news.category, 0) + weight // 2
                
                if news.title:
                    keywords = extract_keywords_from_text(news.title)
                    for kw in keywords:
                        word_counts[kw] = word_counts.get(kw, 0) + weight // 2
                
                if news.summary:
                    keywords = extract_keywords_from_text(news.summary)
                    for kw in keywords:
                        word_counts[kw] = word_counts.get(kw, 0) + weight // 3
    
    behavior_count = len(behaviors)
    news_weight = 1 if behavior_count > 5 else 3
    
    for news in news_items:
        if news.category:
            word_counts[news.category] = word_counts.get(news.category, 0) + news_weight
        
        if news.title:
            keywords = extract_keywords_from_text(news.title)
            for kw in keywords:
                word_counts[kw] = word_counts.get(kw, 0) + news_weight // 2
    
    for category, keywords in category_keywords.items():
        if word_counts.get(category, 0) > 0:
            for kw in keywords[:10]:
                if kw not in word_counts:
                    word_counts[kw] = word_counts.get(category, 0) // 2 + 1
    
    words = []
    for word, count in word_counts.items():
        if word in all_keywords or count >= 2:
...
def get_model_configs(db: Session = Depends(get_session), admin: AdminUser = Depends(get_admin_user)):
    result = []
...
def update_model_config(
    ...
    model_key_prefix = f"model_{request.model_name.lower().replace(' ', '_')}"
    
    for field_key, value in request.config.items():
        key = f"{model_key_prefix}_{field_key}"
        cfg = db.exec(select(SystemConfig).where(SystemConfig.key == key)).first()
        field_info = next((f for f in MODEL_CONFIGS[request.model_name]["fields"] if f["key"] == field_key), None)
        description = field_info["label"] if field_info else field_key
        
        if cfg:
            cfg.value = value
            cfg.description = f"{request.model_name} {description}"
            cfg.updated_at = datetime.utcnow().isoformat()
        else:
            cfg = SystemConfig(
                key=key,
                value=value,
                description=f"{request.model_name} {description}",
            )
            db.add(cfg)
    
    url_key = f"{model_key_prefix}_url"
    url_cfg = db.exec(select(SystemConfig).where(SystemConfig.key == url_key)).first()
    if url_cfg:
        url_cfg.value = request.url
        url_cfg.description = f"{request.model_name} API地址"
        url_cfg.updated_at = datetime.utcnow().isoformat()
    else:
        url_cfg = SystemConfig(
            key=url_key,
            value=request.url,
            description=f"{request.model_name} API地址",
        )
        db.add(url_cfg)
    
    active_key = f"{model_key_prefix}_active"
    active_cfg = db.exec(select(SystemConfig).where(SystemConfig.key == active_key)).first()
    if active_cfg:
        active_cfg.value = "true" if request.is_active else "false"
        active_cfg.description = f"{request.model_name} 是否启用"
        active_cfg.updated_at = datetime.utcnow().isoformat()
    else:
        active_cfg = SystemConfig(
            key=active_key,
            value="true" if request.is_active else "false",
            description=f"{request.model_name} 是否启用",
        )
        db.add(active_cfg)
    
    db.commit()
    
    return {"message": f"{request.model_name} 配置已更新"}
...
def ensure_default_admin(db: Session) -> None:
    ...
    default_config = SystemConfig(
        key="default_api_key",
        value="sk-ca2a253625df40619c2967ef23a4b87d",
        description="默认DeepSeek API密钥",
    )
