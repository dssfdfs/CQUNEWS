...
import { Newspaper, Clock, Network, TrendingUp, Sparkles, ChevronRight, Star, RotateCcw, Crosshair, ExternalLink, ArrowRight, Search } from 'lucide-react';
...
export function NewsRecommend() {
  ...
  const [allNews, setAllNews] = useState<RecommendedNews[]>([]);
  const graphRef = useRef<any>(null);

  const handleNewsClick = (news: RecommendedNews) => {
    recordBehavior({
      action_type: 'view',
      target_id: news.id,
  ...
  useEffect(() => {
    if (showKnowledgeGraph) {
      fetch('/api/news?page_size=200&today_only=true')
        .then(res => res.json())
        .then(result => setAllNews(result.items || []))
        .catch(() => setAllNews([]));
    }
  }, [showKnowledgeGraph]);
  ...
  useEffect(() => {
    if (showKnowledgeGraph && graphRef.current) {
      graphRef.current.cameraPosition(
        { x: cameraX, y: cameraY, z: 1125 },
        { x: cameraX, y: cameraY, z: 0 },
        200
  ...
  }, [cameraX, cameraY, showKnowledgeGraph]);

  const fetchDefaultRecommendations = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/news?page_size=5&today_only=true');
  ...
  const identifyEntityType = (word: string): EntityType => {
    ...
    const commonOrgs = ['腾讯', '阿里巴巴', '百度', '华为', '小米', '京东', '美团', '字节', '滴滴', '网易', '新浪', '搜狐', '网易', '新华网', '人民网', '央视', '新华社', '澎湃', '凤凰网'];
    ...
    const commonGPEs = ['北京', '上海', '广州', '深圳', '成都', '杭州', '南京', '武汉', '重庆', '西安', '天津', '苏州', '郑州', '长沙', '青岛', '沈阳', '大连', '厦门', '香港', '澳门', '台湾', '中国', '美国', '日本', '韩国', '英国', '法国', '德国', '俄罗斯', '印度', '巴西'];
    ...
    const personIndicators = ['先生', '女士', '博士', '教授', '专家', '经理', '主任', '书记', '主席', '局长', '院长', '总理', '部长', '委员', '代表', '记者'];
    
    if (orgPatterns.some(p => p.test(word)) || commonOrgs.some(org => word.includes(org))) return 'ORG';
    if (gpePatterns.some(p => p.test(word)) || commonGPEs.some(gpe => word.includes(gpe))) return 'GPE';
    if (eventPatterns.some(p => p.test(word))) return 'EVENT';
    if (techPatterns.some(p => p.test(word))) return 'TECHNOLOGY';
    if (productPatterns.some(p => p.test(word))) return 'PRODUCT';
  ...
  const generateKnowledgeGraph = async () => {
    ...
    const topicNewsIds = effectiveRecommendations.map(n => n.id);
    nodes.push({ id: 'topic', label: topicLabel, type: 'topic', size: 45, weight: 100, cluster: -1, newsIds: topicNewsIds, isRelevant: true });
    nodeIds.add('topic');
    ...
    try {
      const response = await fetch('/api/news?page_size=100&today_only=true');
      const result = await response.json();
      const newsList = result.items || [];
      setAllNews(newsList);
      
      newsList.forEach((news: RecommendedNews) => {
    ...
    nodes.forEach(node => {
      if (node.entityType && node.entityType !== 'OTHER') {
        entityTypeCounts[node.entityType]++;
      }
    });
    
    const sortedTypes = Object.entries(entityTypeCounts)
      .sort((a, b) => b[1] - a[1]);
    const dominantType = sortedTypes[0]?.[1] > 0 ? sortedTypes[0][0] as EntityType : undefined;
    
    const topicNodeIndex = nodes.findIndex(n => n.id === 'topic');
    if (topicNodeIndex !== -1 && dominantType) {
      nodes[topicNodeIndex].dominantEntityType = dominantType;
    }
      ...
      {showKnowledgeGraph && (
        <div className="mb-6 p-6 bg-white rounded-2xl border border-gray-100 shadow-lg">
              ...
              <button
                onClick={() => {
                  ...
                  if (currentNews && graphRef.current) {
                    const newsNode = knowledgeGraph.nodes.find((n) => n.newsId === Number(currentNews.id));
                    if (newsNode) {
                      const nodeWithCoords = newsNode as unknown as { x: number; y: number; z: number };
                      graphRef.current.cameraPosition(
                        { x: nodeWithCoords.x + 150, y: nodeWithCoords.y + 100, z: 200 },
                        { x: nodeWithCoords.x, y: nodeWithCoords.y, z: nodeWithCoords.z },
                        1000
                      );
                    }
                  }
          ...
          <div 
            ...
            <ForceGraph3D
              ...
              nodeColor={(node) => {
                ...
                if (node.entityType === 'ORG') return '#92400e';
                ...
                if (node.type === 'topic' && node.dominantEntityType) {
                  if (node.dominantEntityType === 'PERSON') return '#ec4899';
                  if (node.dominantEntityType === 'ORG') return '#92400e';
                  if (node.dominantEntityType === 'GPE') return '#10b981';
                  if (node.dominantEntityType === 'EVENT') return '#f59e0b';
                  if (node.dominantEntityType === 'TECHNOLOGY') return '#8b5cf6';
                  if (node.dominantEntityType === 'PRODUCT') return '#06b6d4';
                }
              ...
              nodeThreeObject={(node: KnowledgeGraphNode) => {
            ...
            {selectedNode && (
              ...
              >
                {(() => {
                  ...
                  const getNewsForEntity = () => {
                    if (!node.newsIds || node.newsIds.length === 0) return [];
                    return allNews.filter(n => node.newsIds?.includes(n.id));
                  };
                    ...
                    <div>
                      ...
                      {isNews && node.newsId && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <button
                            onClick={() => {
                              const news = allNews.find(n => n.id === node.newsId);
                              if (news) {
                                handleNewsClick(news);
                              } else {
                                navigate(`/news/${node.newsId}`);
                              }
                            }}
                            className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition-colors"
                          >
                      ...
                      {(isEntity || isTopic || isAttribute) && node.newsIds && node.newsIds.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <div className="text-xs font-medium text-gray-500 mb-2 flex items-center gap-1">
                            <Newspaper className="w-3 h-3" />
                            所属新闻 ({node.newsIds.length})
                          </div>
                          <div className="space-y-1.5">
                            {getNewsForEntity().slice(0, 5).map(news => (
                              <div key={news.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg hover:bg-indigo-50 transition-colors">
                                <button
                                  onClick={() => handleNewsClick(news)}
                                  className="text-xs text-indigo-600 truncate flex-1 mr-2 text-left hover:text-indigo-800 hover:underline"
                                  title={news.title}
                                >
                                  {news.title.length > 20 ? news.title.slice(0, 19) + '...' : news.title}
                                </button>
                                <button
                                  onClick={() => handleNewsClick(news)}
                                  className="flex items-center gap-1 px-2 py-1 bg-indigo-100 text-indigo-600 text-xs rounded hover:bg-indigo-200 transition-colors"
                                >
                            ...
                            {getNewsForEntity().length > 5 && (
                              <div className="text-xs text-center text-gray-400">
                                还有 {getNewsForEntity().length - 5} 条新闻...
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {(isTopic || isAttribute) && !node.newsIds && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <button
                            onClick={() => navigate(`/news?keyword=${encodeURIComponent(node.label)}`)}
                            className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-indigo-50 text-indigo-600 text-xs font-medium rounded-lg hover:bg-indigo-100 transition-colors"
                          >
          ...
          <div className="flex items-center justify-center gap-4 mt-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-50 to-orange-50 rounded-full">
              <div className="w-4 h-4 rounded-full bg-gradient-to-br from-amber-600 to-orange-800 shadow-md" />
              <span className="text-xs font-medium text-amber-800">机构</span>
            </div>
