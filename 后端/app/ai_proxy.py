...
MODEL_CONFIGS = {
    ...
    "豆包": {
        "url": f"{settings.DOUBAO_API_URL}/responses",
    ...
    "千问": {
        ...
        "auth_type": "dashscope",
        "app_id": settings.QWEN_APP_ID,
    },
}

ARK_CONFIG = {
    "base_url": settings.DOUBAO_VIDEO_API_URL,
    "default_api_key": settings.DOUBAO_VIDEO_API_KEY,
    "model_name": settings.DOUBAO_VIDEO_MODEL_NAME,
}
...
async def build_ark_request(config: dict, req: ProcessRequestWithConfig):
    target_url = req.api_url or config["url"]
    target_api_key = req.api_key or config["default_key"]
    target_model = config["model_name"]
    
    if not target_api_key:
        raise HTTPException(status_code=400, detail="请配置豆包API密钥")
...
async def build_dashscope_request(config: dict, req: ProcessRequestWithConfig):
    target_api_key = req.api_key or config["default_key"]
    
    if not target_api_key:
        raise HTTPException(status_code=400, detail="千问需要配置API密钥")
    ...
    return config["url"], payload, headers


async def build_bearer_request(config: dict, req: ProcessRequestWithConfig):
    target_url = req.api_url or config["url"]
    target_api_key = req.api_key or config["default_key"]
    target_model = config["model_name"]
    
    if not target_api_key:
        raise HTTPException(status_code=400, detail="请配置API密钥")
...
async def process_ai_request(
    ...
    try:
        if auth_type == "ark":
            target_url, payload, headers = await build_ark_request(config, req)
        elif auth_type == "dashscope":
            target_url, payload, headers = await build_dashscope_request(config, req)
        else:
            target_url, payload, headers = await build_bearer_request(config, req)
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            ...
            if auth_type == "ark":
                ...
                if isinstance(output, dict):
                    ...
                    content = output[0].get("text", "") if isinstance(output[0], dict) else str(output[0])
                else:
                    content = ""
    ...
    try:
        if auth_type == "ark":
            ...
            payload = {
                ...
                "input": [{
                    "role": "user",
                    "content": [{"type": "input_text", "text": "测试连接"}],
                }],
            }
            headers = {
                "Content-Type": "application/json",
            ...
            if not target_api_key:
                return {"success": False, "message": "千问需要配置API密钥"}
