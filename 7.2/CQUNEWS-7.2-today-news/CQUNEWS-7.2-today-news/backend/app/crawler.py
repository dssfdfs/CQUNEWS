...
def _decode_response(resp: requests.Response) -> str:
    ...
    candidate_results: list[tuple[int, str]] = []
    ...
    for enc in encodings:
        ...
        has_japanese = bool(re.search(r"[\u3040-\u30ff]", decoded))
        has_korean = bool(re.search(r"[\uac00-\ud7af]", decoded))
        ...
        html_tags = len(re.findall(r"<[^>]+>", decoded))
        if html_tags > 10:
            score += 20
        
        candidate_results.append((score, decoded))
    
    if candidate_results:
        candidate_results.sort(key=lambda x: -x[0])
        best_score, best_result = candidate_results[0]
        
        if best_score > 0:
            return best_result
        
        for _, result in candidate_results:
            if not re.search(r"[\x80-\xff]", result):
                continue
            if len(result) > len(raw) * 0.5:
                return result
