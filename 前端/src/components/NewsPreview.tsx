...
const SOURCE_MAP: Record<string, string[]> = {
  '新华网': ['新华网-时政', '新华网-科技', '新华网-国际'],
  '中国新闻网': ['中国新闻网-滚动'],
  '澎湃新闻': ['澎湃新闻'],
};

const PRIMARY_SOURCES = ['新华网-时政', '新华网-科技', '新华网-国际', '中国新闻网-滚动', '澎湃新闻'];
...
export function NewsPreview() {
  ...
  const [selectedSource, setSelectedSource] = useState<(typeof SOURCES)[number]>('全部');
  ...
  const loadNews = async (p = 1, cat = selectedCategory, kw = searchQuery, src = selectedSource) => {
    ...
    try {
      const targetSources = src !== '全部' && src !== '其他' ? SOURCE_MAP[src] || [] : [];
      const needsClientFilter = targetSources.length > 1;
      const singleSource = targetSources.length === 1 ? targetSources[0] : undefined;
      ...
      if (cat === '我的收藏') {
        ...
        const fetchSize = src === '其他' || needsClientFilter ? 100 : pageSize;
        const data = await fetchNews(1, fetchSize, params);
        if (fetchId !== latestFetchId.current) return;
        let allItems = data.items;
        if (src === '其他') {
          allItems = data.items.filter(item => !PRIMARY_SOURCES.includes(item.source || ''));
        } else if (needsClientFilter) {
          allItems = data.items.filter(item => targetSources.includes(item.source || ''));
        }
      ...
      if (src === '其他') {
        ...
        if (kw.trim()) params.keyword = kw.trim();
        const data = await fetchNews(1, 100, params);
        if (fetchId !== latestFetchId.current) return;
        const allItems = data.items.filter(item => !PRIMARY_SOURCES.includes(item.source || ''));
        const startIdx = (p - 1) * pageSize;
        const pageItems = allItems.slice(startIdx, startIdx + pageSize);
        setNews(pageItems);
        setTotal(allItems.length);
      ...
      if (needsClientFilter) {
        ...
        const data = await fetchNews(1, 100, params);
        if (fetchId !== latestFetchId.current) return;
        const allItems = data.items.filter(item => targetSources.includes(item.source || ''));
        const startIdx = (p - 1) * pageSize;
        const pageItems = allItems.slice(startIdx, startIdx + pageSize);
        setNews(pageItems);
        setTotal(allItems.length);
  ...
  const handleRefresh = async () => {
    ...
    try {
      await triggerCrawl();
      await loadNews(1, selectedCategory, searchQuery, selectedSource);
    } catch (err) {
      ...
      <div className={`flex-1 ${showSummaryPanel ? 'max-w-[66.66%]' : ''}`}>
        ...
        {error && (
          <div className="mb-4 flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
            ...
            <button
              onClick={() => loadNews(1, selectedCategory, searchQuery, selectedSource)}
                ...
                {aiSummaryCache[selectedNews.id] ? (
                  <div className="relative">
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <button
                        onClick={() => handleCopySummary(aiSummaryCache[selectedNews.id])}
