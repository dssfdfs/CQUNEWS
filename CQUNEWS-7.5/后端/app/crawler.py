...
def _extract_charset_from_html(raw: bytes) -> str | None:
    try:
        text = raw[:8000].decode("latin-1")
        ...
        content_type_pattern = re.compile(
            r'<meta[^>]+content=["\']?[^"\'>]*charset=([^"\'>;]+)', re.IGNORECASE
        )
        match = content_type_pattern.search(text)
        if match:
            return match.group(1).lower().strip()
    except Exception:
        pass
...
def _is_likely_garbled(text: str) -> bool:
    ...
    common_words = [
        "中国", "新闻", "今日", "时间", "价格", "科学", "技术", "工业", "市场",
        "最后", "最新", "报道", "记者", "发布", "会议", "经济", "社会", "文化",
        "发展", "改革", "政策", "政府", "国家", "人民", "企业", "公司", "集团",
        "国际", "全球", "世界", "美国", "日本", "韩国", "欧洲", "亚洲",
        "北京", "上海", "广州", "深圳", "成都", "杭州", "南京", "武汉",
        "今年", "去年", "明年", "现在", "今天", "昨天", "明天",
        "亿元", "万", "千", "百", "十", "一",
        "说", "是", "在", "有", "和", "了", "的", "与", "及", "等",
        "月", "日", "时", "分", "秒", "年",
        "地震", "比赛", "球队", "足球", "电商", "退换货", "升级", "体验",
        "辟谣", "出局", "痛哭", "爆冷", "影响", "首次", "基金会",
        "委内瑞拉", "埃菲尔", "巴菲特", "盖茨", "内马尔", "铁路", "货物",
        "发送", "热浪", "青年", "锦标赛", "男足", "开门红", "友谊赛",
        "A股", "交易", "新规", "落地", "执行", "机器人", "出海", "产品",
        "方案", "也门", "政府军", "胡塞武装", "袭击", "死亡", "抖音",
        "运费险", "首重", "公斤", "提升", "用户", "年薪", "抢人",
        "红牌", "暂缓", "全世界", "目瞪口呆",
    ]
    found_count = 0
    ...
    uncommon_chinese = re.findall(r"[\u5b81\u5099\u659c\u6e58\u91cf\u8f85\u901a\u6307\u804c\u6e05\u964b\u539f\u6839\u6700\u7ec8]", text)
    if len(uncommon_chinese) > 3:
        return True
...
def _decode_response(resp: requests.Response) -> str:
    ...
    charset_from_html =_extract_charset_from_html(raw)
    if charset_from_html:
        priority_encodings.append(charset_from_html)

    if resp.encoding:
        priority_encodings.append(resp.encoding)

    standard_order = ["utf-8", "gb18030", "gbk", "gb2312", "big5", "latin-1"]

    if resp.apparent_encoding:
        priority_encodings.append(resp.apparent_encoding)
    ...
    ascii_only_fallback: str | None = None
    latin_aliases = {"latin-1", "iso-8859-1", "iso8859-1", "8859-1"}
    ...
    for enc in encodings:
        ...
        if "\ufffd" in decoded:
            continue
        has_chinese = bool(re.search(r"[\u4e00-\u9fff]", decoded))
        if has_chinese and not _is_likely_garbled(decoded):
            return decoded
        if enc in ("utf-8", "gb18030", "gbk", "gb2312", "big5"):
            if has_chinese:
                fallback = decoded
        elif enc in latin_aliases:
            if not re.search(r"[^\x00-\x7f]", decoded):
                ascii_only_fallback = decoded
            else:
                fallback = decoded

    if not fallback or not re.search(r"[\u4e00-\u9fff]", fallback or ""):
        for enc in ["utf-8", "gb18030", "gbk", "gb2312"]:
            ...
            has_chinese = bool(re.search(r"[\u4e00-\u9fff]", decoded))
            if has_chinese and not _is_likely_garbled(decoded):
                if best_candidate is None or replacement_count < best_candidate[0]:
                    best_candidate = (replacement_count, decoded)

    if best_candidate:
        return best_candidate[1]
    if fallback:
        return fallback
    if ascii_only_fallback:
        return ascii_only_fallback
...
def fetch_article(session: requests.Session, url: str, source_name: str, category: str, list_title: str = "") -> CrawledItem | None:
    try:
        ...
        if not title or len(content) < 40:
            if list_title and len(list_title) >= 6:
                title = list_title
                if len(text) >= 40:
                    summary = text[:120]
                    content = text[:8000]
                ...
                return None
        if list_title and _is_likely_garbled(title):
            title = list_title
...
def crawl_source(source: CrawlSource, max_articles: int = 15) -> CrawlResult:
    ...
    try:
        ...
        for title_text, abs_url in links[:max_articles]:
            _polite_sleep()
            item = fetch_article(session, abs_url, source.name, source.category or "", title_text)
            if item is not None:
