...
export interface NewsCategoryData {
  category_distribution: Array<{ name: string; value: number; percentage: number }>;
  total: number;
}
...
export const userApi = {
  ...
  getFavorites: async () => {
    const token = localStorage.getItem('token');
    const response = await fetch('/api/news/favorite/list', {
      headers: {
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
      },
    });
  ...
  recordBehavior: async (actionType: string, targetId?: number, extraData?: Record<string, unknown>) => {
    ...
    const response = await fetch('/api/behavior/record', {
      ...
      body: JSON.stringify({
        ...
        target_id: targetId,
        category: extraData?.category || '综合',
...
export const adminApi = {
  ...
  getNewsCategory: async (days: number = 30): Promise<NewsCategoryData> => {
    const response = await fetch(`/api/admin/analytics/news-category?days=${days}`, {
      headers: adminHeaders(),
    });
    return response.json();
  },
  ...
  getDailyStats: async (days: number = 7): Promise<{ data: Array<{ date: string; day_name: string; generate_count: number; view_count: number; share_count: number; feedback_count: number; active_users: number }> }> => {
  ...
  batchDeleteUsers: async (userIds: number[]): Promise<{ message: string }> => {
  ...
  updateModelConfig: async (modelName: string, config: Record<string, string>, url: string, isActive: boolean): Promise<{ message: string }> => {
    const response = await fetch('/api/admin/config/model', {
      method: 'PUT',
      headers: adminHeaders(),
      body: JSON.stringify({ model_name: modelName, config, url, is_active: isActive }),
    });
    return response.json();
  },

  testModelConnection: async (modelName: string, config: Record<string, string>, url: string): Promise<{ success: boolean; message: string }> => {
    const response = await fetch('/api/admin/config/model/test', {
      method: 'POST',
      headers: adminHeaders(),
      body: JSON.stringify({ model_name: modelName, config, url }),
    });
    return response.json();
  },
