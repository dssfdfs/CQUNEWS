...
export function AdminUserManagement({ activeItem, onItemClick }: AdminUserManagementProps) {
  ...
  const [userWordCloud, setUserWordCloud] = useState<UserWordCloudItem[]>([]);
  ...
  const handleUserClick = async (user: UserInfo) => {
    ...
    try {
      ...
      setUserWordCloud(wordCloudData.words.slice(0, 3).map((t: { text: string; value: number; category: string }) => ({ word: t.text, weight: t.value })));
                    ...
                    {userWordCloud.length === 0 ? (
                      ...
                      <div className="flex flex-wrap gap-3">
                        {userWordCloud.map((tag, index) => (
                          ...
                          >
                            {tag.word} ({Math.round(tag.weight)})
                          </span>
                        ))}
                      </div>
                    )}
                      ...
                      <div className="space-y-3">
                        {userHistory.slice(0, 10).map((item) => (
                          <div key={item.id} className="p-4 bg-gray-50 rounded-lg">
                            ...
                            {typeof (item.metadata as Record<string, unknown>)?.duration_ms === 'number' && (
                              <div className="mt-2 pt-2 border-t border-gray-200">
                                <p className="text-sm text-gray-600">
                                  耗时 {String((item.metadata as Record<string, unknown>).duration_ms)}ms
                                </p>
                              </div>
                            )}
                          </div>
