...
export async function callDeepSeek(model: string, messages: Message[], apiConfig?: ApiConfig, temperature = 0.7, max_tokens = 2000): Promise<string> {
  ...
  if (apiConfig && apiConfig.apiUrl && apiConfig.apiUrl.trim() && apiConfig.apiUrl !== '/api/process') {
  ...
  if (!response.ok) {
    ...
    const errorMessage = errorData.detail || errorData.message || errorData.error?.message || `HTTP error! status: ${response.status}`;
