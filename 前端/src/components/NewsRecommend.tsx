...
export function NewsRecommend() {
  ...
  const generateKnowledgeGraph = async () => {
    ...
    const sortedTypes = Object.entries(entityTypeCounts)
      .sort((a, b) => b[1] - a[1]);
    const dominantType = sortedTypes[0]?.[1] > 0 ? sortedTypes[0][0] : undefined;
                    ...
                    <div>
                      ...
                      {isNews && node.newsId && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <button
                            onClick={() => { recordBehavior({ action_type: 'view', target_id: node.newsId }); navigate(`/news/${node.newsId}`); }}
                          ...
                          <div className="space-y-1.5">
                            {getNewsForEntity().slice(0, 5).map(news => (
                              <div key={news.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg hover:bg-indigo-50 transition-colors">
                                <button
                                  onClick={() => { recordBehavior({ action_type: 'view', target_id: news.id, category: news.category, title: news.title }); navigate(`/news/${news.id}`); }}
