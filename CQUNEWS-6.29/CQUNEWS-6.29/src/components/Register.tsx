...
import { Sparkles, User, Mail, Lock, Eye, EyeOff, CheckCircle, RefreshCw } from 'lucide-react';
import { getCaptcha, register } from '@/api/deepseek';
...
export function Register() {
  ...
  const registerStore = useStore((state) => state.register);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await register(username, email, password, captcha, sessionKey);
      const userInfo = {
      ...
      registerStore(username, email, password, userInfo);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-3">
                ...
                <div className="relative">
                  <div className="flex items-center justify-center w-32 h-12 bg-gray-100 rounded-lg border border-gray-300">
                    <span className="text-lg font-mono font-bold text-gray-700 tracking-wider">{captchaCode}</span>
                  </div>
                  <button
                    type="button"
                    onClick={loadCaptcha}
